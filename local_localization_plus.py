##添加功能：1.可以根据机器人行径过程中遇到的不同mark贴纸自动更改landmark 注：因为是一维，所以最好是只有一个mark作为衡量即可
##         2.不断根据不同的mark 创建不同的局部地图，同时也更改粒子的位置（更改到小车到landmark之间）
##         3.可以输出小车到mark的大致距离，单位cm，四舍五入为整数
##         4.如果行径过程没有看到mark，所有粒子将保持上一轮的权值，并全部进行practice。
## 尚未调参。
...
def run_pf(N, sensor_std_err=0.1):
    ...
    landmarks =[]
    while True :
        while (corners is None)and(timer1 != 10): ##直到摄像头可以看到mark，并得到数据
           camera.capture(frame, use_video_port=True, format='bgr')
           targetID2 = targetID
           targetID = detector.Getid(frame)
           ...
           if(corners is not None):
              ...
              if(targetID2 != targetID):
                 ...
                 nowid=targetID
        print("Now mark is %s"%(targetID))
        # create particles and weights
        max = round(marklocal)## max的值，需要具体接口知道实际地图的大小
        particles = create_uniform_particles(max, N)
        weights = np.zeros(N)
        robot_pos = np.array(0.)
        xs = []
        print("Create maps of 0 to %s"%(max))
        key = readkey()
        if key=='p':
            print("stop working")
            sys.exit(0)
        ...
        corners = None
        timer2 = 0
        while(corners is None)and(timer2 != 10): ##直到摄像头可以看到mark，并得到数据
            camera.capture(frame, use_video_port=True, format='bgr')
            targetID = detector.Getid(frame)
            corners = detector.detectMarker(frame, targetID)
            timer2 +=1
