...
static const char *log_tag = "video-transmission";
/*
 * Private methods
 */
...
static void check_initialization_complete(CustomData *data) {
    ...
    if (!data->initialized && data->native_window && data->main_loop) {
        GST_DEBUG("Initialization complete, notifying application. native_window:%p main_loop:%p", data->native_window,
                  data->main_loop);
...
static void gst_native_surface_init(JNIEnv *env, jobject thiz, jobject surface) {
    ...
    if (data->native_window) {
        ...
        if (data->native_window == new_native_window) {
            GST_DEBUG("New native window %p is the same as the previous one", data->native_window);
            if (data->video_sink) {
